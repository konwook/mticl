...
from tianshou.utils.net import discrete
...
class BasePolicy(ABC, nn.Module):
    ...
    * :meth:`~srl.policy.BasePolicy.__init__`: initialize the policy, including \
        coping the target network and so on;
    * :meth:`~srl.policy.BasePolicy.forward`: compute action with given \
        observation;
    * :meth:`~srl.policy.BasePolicy.process_fn`: pre-process data from the \
        replay buffer (this function can interact with replay buffer);
    * :meth:`~srl.policy.BasePolicy.learn`: update policy with a given batch of \
        data.
    * :meth:`~srl.policy.BasePolicy.post_process_fn`: update the replay buffer \
        from the learning process (e.g., prioritized replay buffer needs to update \
        the weight);
    * :meth:`~srl.policy.BasePolicy.update`: the main interface for training, \
    ...
    Since :class:`~saferl.policy.BasePolicy` inherits ``torch.nn.Module``, you can
    use :class:`~saferl.policy.BasePolicy` almost the same as ``torch.nn.Module``,
    ...
    def __init__(
        ...
        max_batchsize: Optional[int] = 256,
        ...
        if isinstance(action_space, (Discrete, MultiDiscrete, MultiBinary)):
            ...
            print("Warning! The action space type is unclear, regard it as continuous.")
            self.action_type = "continuous"
            print(self.action_space)
        if isinstance(actor, discrete.Actor):
            self.action_type = "discrete"
        self.updating = False
    ...
    def pre_update_fn(self, **kwarg: Any) -> Any:
        ...
        This function is called after each data collection in :meth:`trainer`
        and could be used to update the Lagrangian multiplier.
        """
        pass

    def post_update_fn(self, **kwarg: Any) -> Any:
        """Post-process the policy or data after updating the policy.
        This function is in :meth:`trainer` and could be used to
        sync the weight or old variables.
        """
        pass
    ...
    def map_action_inverse(
        ...
        """Inverse operation to :meth:`~saferl.policy.BasePolicy.map_action`.
    ...
    def get_metrics(batch: Batch):
        cost = batch.info["cost"].astype(batch.rew.dtype)
    ...
    def compute_gae_returns(
        ...
        for i in range(self.critics_num):
            ...
            # when normalizing values, we dont minus self.ret_rms.mean to be numerically
            # consistent with OPENAI baselines' value normalization pipeline. Emperical
            # study also shows that "minus mean" will harm performances a tiny bit
