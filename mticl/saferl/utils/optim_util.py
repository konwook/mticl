...
class LagrangianOptimizer(object):
    ...
    .. note::
        ...
        If kp and kd are 0, it reduced to a standard SGD.
    """

    def __init__(self, pid: list = [0.05, 0.005, 0]) -> None:
        super().__init__()
    ...
    def update_expert_cost(self, expert_cost):
        print(f"Expert cost: {expert_cost}")
    ...
    def step(self, value, threshold):
        """Optimize the multipler by one step

        :param float value
        :param float threshold
        """
        error_new = np.mean(value - threshold)
        if self.expert_cost is not None:
            error_new = np.mean(value - threshold - self.expert_cost)
